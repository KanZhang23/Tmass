...
** $Header: /local/packages/gnu/tcl/signal_ext1.4/RCS/signal_ext.c,v 1.6 2003/10/29 05:25:17 mschwart Exp mschwart $
...
static char *SignalExtensionVersion = "1.4.1";
...
static int PrintSignalHandler (ClientData d, Tcl_Interp *i, 
				int argc, const char *argv[]);
static int MyWaitpidKluge (ClientData d, Tcl_Interp *i, 
			   int argc, const char *argv[]);
static int MyGetppidKluge (ClientData d, Tcl_Interp *i, 
			   int argc, const char *argv[]);
...
int Signal_ext_Init ( Tcl_Interp *interp )
{
  ...
  if (initialized == 0)
  {
    ...
    Tcl_CreateFileHandler(fds[0], TCL_READABLE,
                          HandleSignalPipe, (ClientData)interp);

    /* Add the signal command */
    Tcl_CreateCommand(interp, "signal", DoSignalHandler, 0, 0);

    Tcl_CreateCommand(interp, "waitpid", MyWaitpidKluge, 0, 0);
    Tcl_CreateCommand(interp, "getppid", MyGetppidKluge, 0, 0);
  }
...
static int handle_async (ClientData clientData, Tcl_Interp *i, int code)
{
  ...
  */
  ...
  if (interp)
  {
    ...
    {
        const char* code = Tcl_GetVar(interp, "errorCode", TCL_GLOBAL_ONLY);
        if (code)
            errorcode = strdup(code);
        else
            errorcode = strdup("");
    }
    {
...
static int MyWaitpidKluge (ClientData d, Tcl_Interp *interp, 
			   int argc, const char *argv[])
{
    int pid;
    char buffer[32];
	...
	return TCL_ERROR;
    sprintf(buffer, "%d", (int)waitpid((pid_t)pid, NULL, WNOHANG));
    Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    return TCL_OK;
}

static int MyGetppidKluge (ClientData d, Tcl_Interp *interp, 
			   int argc, const char *argv[])
{
    char buffer[32];
    if (argc != 1)
