...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(CurvesWidget, curves.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(CurvesWidget, curves.psFontSize), XmRString, "0"},
    ...
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(CurvesWidget, curves.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(CurvesWidget, curves.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(CurvesWidget, curves.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(CurvesWidget, curves.addTop), XmRString, "0"}
};
...
static void initialize(CurvesWidget request, CurvesWidget new)
{
    ...
    if (new->curves.psFont != NULL)
    	new->curves.psFont = XmStringCopy(new->curves.psFont);
...
static void destroy(CurvesWidget w)
{
    ...
    if (w->curves.psFont != NULL)
      XmStringFree(w->curves.psFont);
...
static void resize(CurvesWidget w)
{
	...
	w->curves.addLeft, w->curves.addRight,
	w->curves.addTop, w->curves.addBottom,
	&w->curves.xMin, &w->curves.yMin, &w->curves.xMax, &w->curves.yMax);
    	...
    	if (sw->curves.updateMarkLine == NULL) {
          ...
          for (j = 0, i = 0; i < nCurves; i++) {
            if ((sw->curves.data+i)->options == CURVE_NO_OPTIONS) {
              if ( (unsigned)j > XtNumber(lines) -2)
        	j = 0;
...
static void redisplayContents(CurvesWidget w, int outDevice, int redrawArea)
{
	...
	if (redrawArea == REDRAW_ALL || w->curves.doubleBuffer) {
    ...
    if (w->curves.isEmptyData) 
      ...
      DrawHorizontalAxis(display, drawBuf, gc, fs, outDevice,
	      ...
	      w->curves.maxXLim, w->curves.xLogScaling, x_method,
              w->curves.psFont, w->curves.psFontSize);
    	...
    	else
	{
	    char *labelfont = NULL;
	    if (w->curves.psFont)
		XmStringGetLtoR(w->curves.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
	    PSDrawXmString(display, drawBuf, w->curves.font, w->curves.xAxisLabel,
		gc, w->curves.xOrigin, w->curves.axisBottom + X_LABEL_MARGIN,
		w->curves.xEnd - w->curves.xOrigin, XmALIGNMENT_CENTER,
                labelfont, w->curves.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
    }
...
static void drawCurvesPlot(CurvesWidget w, Drawable drawBuf, int outDevice){
  ...
  for (i = 0; i < (int)(strlen(w->curves.y_list)); i++){
    current_y = w->curves.y_list[i] - 'A';
    if ((w->curves.data+current_y)->num_point > 0) {
     ...
     XmDrawingAreaCallbackStruct *call_data;
{
  ...
  curve_num = 0;
  for (i = 0; i < (int)(strlen(data->curves.y_list)); i++) {
...
static void create_command_area(Widget parent, CurvesWidget data) {
  ...
  for (i = 0; (unsigned)i < XtNumber(CURVES_COMMAND); i++) {
    XtSetArg(wargs[0], XmNuserData, (XtPointer)((long)i));
...
static void create_selection_area(Widget parent, CurvesWidget data) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((data->curves.data+current_y)->options != CURVE_NO_OPTIONS)
      continue;
    n1 = 0;
    XtSetArg(wargs[n1], XmNuserData, (XtPointer)((long)i)); n1 ++;
    if (i == 0) {
...
static void create_mark_area(Widget parent, CurvesWidget data) {
  ...
  /* create buttons */
  for (i = 0; (unsigned)i < XtNumber(symbol); i++) {
    ...
    XtSetArg(wargs[n], XmNuserData, (XtPointer)((long)i)); n++;
    if (i == mark_num) {
...
static void create_line_area(Widget parent, CurvesWidget data) {
  ...
  /* create buttons */
  for (i = 0; (unsigned)i < XtNumber(lines); i++) {
...
static void setup_setting_widget(Widget w) {
  ...
  XtVaSetValues(topshell, XmNshadowThickness, 0, NULL);
     ...
     XmDrawingAreaCallbackStruct *call_data;
{
  ...
  curve_num = 0;
  for (i = 0; (unsigned)i < strlen(data->curves.y_list); i++) {
    current_y = (int)(data->curves.y_list[i]-'A');
    if ((data->curves.data+current_y)->options != CURVE_NO_OPTIONS)
    	continue;
...
{
    Arg wargs[2];
    int userData, scale_error_bars, current_y, i;
    double value;
    XtPointer p;
    ...
    XtSetArg(wargs[0], XmNuserData, &p);
    XtGetValues(w, wargs, 1);
    userData = (long)p;
...
static void scale_variable(w, data, call_data)
     ...
     XmToggleButtonCallbackStruct *call_data;
  {
    ...
    XtSetArg(wargs[0], XmNuserData, &userDa);
    XtGetValues(w, wargs, 1);
    user_data = (long)userDa;
...
static void create_scaling_area(Widget parent, CurvesWidget data) {
  ...
  /* create sub_widgets */
  for (i = 0; (unsigned)i < XtNumber(scaleName); i++) {
    XtSetArg(wargs[0], XmNuserData, (XtPointer)((long)i));
    if (i <= (int)(XtNumber(scaleName)) - 4) 
