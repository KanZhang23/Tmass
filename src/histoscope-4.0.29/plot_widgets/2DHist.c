...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(Hist2DWidget, hist2D.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(Hist2DWidget, hist2D.psFontSize), XmRString, "0"},
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(Hist2DWidget, hist2D.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(Hist2DWidget, hist2D.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(Hist2DWidget, hist2D.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(Hist2DWidget, hist2D.addTop), XmRString, "0"}
};
  ...
  Cardinal*  num_args)
{
    ...
    if (new->hist2D.psFont != NULL)
	new->hist2D.psFont = XmStringCopy(new->hist2D.psFont);
...
static void destroy(Widget wg)
{
    ...
    if (w->hist2D.psFont != NULL)
	XmStringFree(w->hist2D.psFont);
}
...
static void resize(Widget wg)
{
	...
	cbStruct.wingc = w->hist2D.clippingGc;
...
/*	        	 XClearWindow(XtDisplay(w),XtWindow(w)); */
			XPutImage(dpy,drawBuf,w->hist2D.gc,
				  w->hist2D.wireImage, 
				  0,0,0,0 /*
					    w->hist2D.ImageStart,
					    w->hist2D.ImageEnd */,
			    ...
			    ;
			drawArrows(dpy,drawBuf,
				   w->hist2D.currentPicture,w->hist2D.gc);
			drawTics(dpy,drawBuf,
				 w->hist2D.currentPicture,w->hist2D.gc);
			drawNames(dpy,drawBuf,
				  w->hist2D.currentPicture,w->hist2D.gc);
		    }
	    }
            else if (w->hist2D.templateDragging && !w->hist2D.badPicture)
            {
	        drawTemplate(dpy,drawBuf,w->hist2D.currentPicture,
			     w->hist2D.gc, w->hist2D.backPlaneGc);
            }
            else 
	    {
		XFillRectangle(dpy, drawBuf, w->hist2D.inverseGc, 
	...
	do {
        ...
        if (w->hist2D.doubleBuffer)
    	    XCopyArea(dpy, drawBuf, XtWindow(w), w->hist2D.gc, 0, 0,
