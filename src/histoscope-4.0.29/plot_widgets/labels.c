...
static labelTable *makeLableTable(double *value, unsigned inputN, lCS *labelCS)
{
    const int n = inputN;
...
/*  add Motif Shadow size here				*/
    w->hist2D.topMargin    = MARGIN + (w->hist2D.addTop > 0 ? w->hist2D.addTop : 0);
    w->hist2D.bottomMargin = MARGIN + (w->hist2D.addBottom > 0 ? w->hist2D.addBottom : 0);
    w->hist2D.leftMargin   = MARGIN + (w->hist2D.addLeft > 0 ? w->hist2D.addLeft : 0);
    w->hist2D.rightMargin  = MARGIN + (w->hist2D.addRight > 0 ? w->hist2D.addRight : 0);
