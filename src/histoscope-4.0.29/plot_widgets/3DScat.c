...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(Scat3DWidget, scat3D.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(Scat3DWidget, scat3D.psFontSize), XmRString, "0"},
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(Scat3DWidget, scat3D.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(Scat3DWidget, scat3D.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(Scat3DWidget, scat3D.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(Scat3DWidget, scat3D.addTop), XmRString, "0"}
};
  ...
  Cardinal*  num_args)
{
    ...
    if (new->scat3D.psFont != NULL)
	new->scat3D.psFont = XmStringCopy(new->scat3D.psFont);
...
/* setup dashed line for back edges drawing, if requested. Do it
   only once at startup and don't bother any more 			*/  
    ...
    new->scat3D.rightMargin  = MARGIN + (new->scat3D.addRight > 0 ? new->scat3D.addRight : 0);
    new->scat3D.leftMargin   = MARGIN + (new->scat3D.addLeft > 0 ? new->scat3D.addLeft : 0);
    new->scat3D.topMargin    = MARGIN + (new->scat3D.addTop > 0 ? new->scat3D.addTop : 0);
    new->scat3D.bottomMargin = MARGIN + (new->scat3D.addBottom > 0 ? new->scat3D.addBottom : 0);
...
static void destroy(Widget wg)
{
    ...
    if (w->scat3D.psFont != NULL)
	XmStringFree(w->scat3D.psFont);
}
...
static void resize(Widget wg)
{
	...
	cbStruct.wingc = w->scat3D.gc;
	cbStruct.plotgc = NULL;
    	XtCallCallbacks(wg, XmNresizeCallback, &cbStruct);
    }
}
  ...
  Scat3DRescaleModes rescale)            
{
    ...
    if (rescale != SCAT3D_NO_RESCALE && rescale != SCAT3D_REFRESH)
    {
...
static void finishDrawing(Scat3DWidget w)
{
	...
	do {
		...
		cbStruct.drawBuf = w->scat3D.drawBuffer;
	    else
		cbStruct.drawBuf = XtWindow((Widget)w);
	    cbStruct.wingc = w->scat3D.gc;
	    cbStruct.plotgc = NULL;
...
/* change mode to a single point plotting for printing			*/
    ...
    if (w->scat3D.psFont)
    {
	XmStringGetLtoR(w->scat3D.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
	psfontsize = w->scat3D.psFontSize;
    }
    for (i=0; i<3; i++)
        if (w->scat3D.axis[i] != NULL) 
               makePSAxis(w->scat3D.axis[i], psfile, labelfont, psfontsize);
    if (labelfont)
	XtFree(labelfont);
...
/* change mode to initial						*/
	...
	cbStruct.reason = XmCR_EXPOSE;
	cbStruct.event = NULL;
	cbStruct.outDevice = PS_PRINTER;
	cbStruct.xform = NULL;
	cbStruct.drawBuf = XtWindow(wg);
	cbStruct.wingc = w->scat3D.gc;
	cbStruct.plotgc = NULL;
	XtCallCallbacks(wg, XmNredisplayCallback, (XtPointer)&cbStruct);
    } while(0);
}
/*
