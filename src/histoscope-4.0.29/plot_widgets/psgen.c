...
/* set standard scale for makePSTics fuctions (1 pixel == 1 PS unit) 	*/
	...
	cbStruct.drawBuf = XtWindow(wid);
	cbStruct.wingc = w->hist2D.clippingGc;
	cbStruct.plotgc = NULL;
    	XtCallCallbacks(wid, XmNredisplayCallback, &cbStruct);
    } while(0);
}
...
static void drawLabels (Hist2DWidget w, FILE *psfile)
{
    ...
    if (w->hist2D.psFont)
	XmStringGetLtoR(w->hist2D.psFont, XmSTRING_DEFAULT_CHARSET, &font);
    if (w->hist2D.psFont && w->hist2D.psFontSize)
	fontsize = w->hist2D.psFontSize;

    if (font)
	fprintf(psfile,"/%s findfont \n", font);
    else
	fprintf(psfile,"/Times-Roman-ISOLatin1 findfont \n");
    if (fontsize)
	fprintf(psfile,"%d \n", fontsize << LOWBITS);
    else
    {
...
/* set standard scale for makePSTics fuctions (1 pixel == 1 PS unit) 	*/
        fprintf(psfile,"gsave %d dup scale  			\n",
          MAXPIC);
 	makePSLabels (fs, oD->xSegment, psfile, font, fontsize);
 	makePSLabels (fs, oD->ySegment, psfile, font, fontsize);
        fprintf(psfile,"grestore 				\n");
    }
