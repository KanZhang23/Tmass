...
static void drawLinearVAxis(Display *display, Drawable window, GC gc,
	...
	XmString psfont, int psfontsize);
...
static void drawVCenteredFloat(Display *display, Drawable window, GC gc,
	XFontStruct *fs, int outDevice, double value, int x, int y,
	int maxWidth, int maxDecimalOffset, XmString psfont, int psfontsize);
...
void DrawHorizontalAxis(Display *display, Drawable window, GC gc,
	XFontStruct *fs, int outDevice, int y, int x1, int x2,
	double minData, double maxData, double minLimit, double maxLimit,
	int logScaling, int nBins, XmString psfont, int psfontsize)
{
    ...
    if (logScaling) {
    	drawLogHAxis(display, window, gc, fs, outDevice, y, x1, x2,
    		minData, maxData, minLimit, maxLimit, psfont, psfontsize);
    	return;
    }
    ...
    if (nBins != 0) {
    	...
    	if (nPixelsPerBin > BIN_TIC_THRESHOLD) {
    	    drawDiscreteHAxis(display, window, gc, fs, outDevice, y, x1, x2,
    		    minData, maxData, minLimit, maxLimit, nBins, psfont, psfontsize);
    	    return;
    	}
    }
...
**			server for the font data
**	outDevice	Output device, X_SCREEN, or PS_PRINTER
**	x, y1, and, y2	Where to draw the axis line itself
**			in window coordinates.
        ...
        XmString psfont, int psfontsize)
{
	...
	minData<-DBL_MAX || maxData>DBL_MAX ||
	minLimit<-DBL_MAX || maxLimit>DBL_MAX ||
	maxData-minData>DBL_MAX || maxLimit-minLimit>DBL_MAX)
    	return;
        ...
        XmString psfont, int psfontsize)    
{
    ...
    double scale, maxTics, ticUpLim, ticLoLim;
    double ticY;
	...
	/* igv: due to round-off errors, labels which should be 0
           sometimes appear as annoying small numbers. Fix that. */
	if (fabs(labelValue) < fabs(interval/10000.0))
	    labelValue = 0.0;
	...
	if (fabs(labelValue) < fabs(interval/10000.0))
    	...
    	if (labelCenter >= y1 && labelCenter <= y2) {
    	    drawVCenteredFloat(display, window, gc, fs, outDevice, labelValue,
    	    	   ...
    	    	   maxDecimalOffset, psfont, psfontsize);
    	}
    }

    /* igv: use rint since "ticsBetweenLabels" is an int! */
    ticsBetweenLabels = rint(interval/ticInterval);
    ...
    ticUpLim = y2 + ticInterval/10000.0;
    ticLoLim = y1 - ticInterval/10000.0;
    for (i=0; i<=maxTics; i++) {
    	ticValue = tic1Value + ticInterval * i;
    	ticY = y2 - (ticValue - minLimit) * scale;
    	if (ticY >= ticLoLim && ticY <= ticUpLim) {
	    if (ticY < y1) ticY = y1;
	    if (ticY > y2) ticY = y2;
	...
	for (j=1; labelValue < labelLimit; j++) {
    	...
    	if (powLabelCenter >= y1 && powLabelCenter <= y2)
    	    drawVCenteredFloat(display, window, gc, fs, outDevice,
    	    	    ...
    	    	    maxLabelWidth, maxDecimalOffset, psfont, psfontsize);
        ...
        XmString psfont, int psfontsize)
{
    ...
    double labelValue, label1Value, ticValue, tic1Value, ticUpLim, ticLoLim;
	...
	if (fabs(labelValue) < fabs(interval/10000.0))
    	...
    	if (labelCenter >= x1 && labelCenter <= x2) {
    	    drawHCenteredFloat(display, window, gc, fs, outDevice,
    	    	labelValue, labelCenter, labelY, psfont, psfontsize);
    	}
    }
    ...
    /* draw the tics */
    ticUpLim = x2 + ticInterval/10000.0;
    ticLoLim = x1 - ticInterval/10000.0;
    for (i=0; i<=maxTics; i++) {
    	ticValue = tic1Value + ticInterval * i;
    	ticX = minPix + (ticValue - minData) * scale;
    	if (ticX >= ticLoLim && ticX <= ticUpLim) {
	    if (ticX < x1) ticX = x1;
	    if (ticX > x2) ticX = x2;
    	    if (i % ticsBetweenLabels == 0)
    		ticLen = LONG_TIC_LEN;
        ...
        XmString psfont, int psfontsize)
{
    ...
    for (i=0; i<=maxPowLabels; i++) {
    	...
    	if (labelCenter >= x1 && labelCenter <= x2)
    	    drawHCenteredFloat(display, window, gc, fs, outDevice,
    	    	powLabelValue, labelCenter, labelY, psfont, psfontsize);
    	if (interval > 5.)
    	    continue;
	if (interval > 1.) {
...
*/
static double calcLabelInterval(int pixRange, double dataRange,
				XFontStruct *fs, int *factor)
{
        ...
        XmString psfont, int psfontsize)
{
	...
	if (psfont)
	{
	    XmStringGetLtoR(psfont, XmSTRING_DEFAULT_CHARSET, &font);
            fontsize = psfontsize;
	}
		...
		int y, int drawAxisLine, int drawLeftArrow, int drawRightArrow)
{
    ...
    if (drawLeftArrow && isLeftArrowEnabled()) {
    	seg->x1=x1; seg->x2=x1+6; seg->y1=y; seg->y2=y+2; seg++; nSegs++;
    	seg->x1=x1; seg->x2=x1+6; seg->y1=y; seg->y2=y-2; seg++; nSegs++;
    }
    if (drawRightArrow && isRightArrowEnabled()) {
	...
	int y2, int drawAxisLine, int drawTopArrow, int drawBottomArrow)
{
    ...
    /* draw arrow heads to indicate data not shown */
    if (drawTopArrow && isTopArrowEnabled()) {
    	seg->y1=y1; seg->y2=y1+6; seg->x1=x; seg->x2=x+2; seg++; nSegs++;
    	seg->y1=y1; seg->y2=y1+6; seg->x1=x; seg->x2=x-2; seg++; nSegs++;
    }
    if (drawBottomArrow && isBottomArrowEnabled()) {
	...
	int maxWidth, int maxDecimalOffset, XmString psfont, int psfontsize)
{
	...
	if (psfont)
	{
    	...
    	PSDrawString(display, window, gc, fs, x-offset, top,
		     PS_RIGHT, valueStr, font, fontsize);
	if (font)
	    XtFree(font);
    }
}
	...
	XFontStruct *fs)
{
    ...
    char mantString[16], expString[16];
    int expLen, prec;
