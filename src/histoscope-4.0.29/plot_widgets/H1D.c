...
static void btn3AP(H1DWidget w, XEvent *event, char *args, int n_args);
...
static void computeTransform(H1DWidget w, XYTransform *xform);
...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(H1DWidget, h1D.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(H1DWidget, h1D.psFontSize), XmRString, "0"},
    ...
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(H1DWidget, h1D.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(H1DWidget, h1D.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(H1DWidget, h1D.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(H1DWidget, h1D.addTop), XmRString, "0"}
};
...
static void initialize(H1DWidget request, H1DWidget new)
{
    ...
    if (new->h1D.psFont != NULL)
	new->h1D.psFont = XmStringCopy(new->h1D.psFont);
...
static void destroy(Widget w)
{
    ...
    if (hw->h1D.psFont != NULL)
        XmStringFree(hw->h1D.psFont);
}
...
static void resize(Widget w)
{
	...
	hw->h1D.addLeft, hw->h1D.addRight, hw->h1D.addTop, hw->h1D.addBottom,
	&hw->h1D.xMin, &hw->h1D.yMin, &hw->h1D.xMax, &hw->h1D.yMax);
    ...
    if (hw->h1D.xOrigin > hw->h1D.xMin + (hw->h1D.xMax - hw->h1D.xMin) * MAX_AXIS_PERCENT/100)
    	hw->h1D.xOrigin = hw->h1D.xMin + (hw->h1D.xMax - hw->h1D.xMin) * MAX_AXIS_PERCENT/100;
	...
	XYTransform xform;
	computeTransform(hw, &xform);
	cbStruct.reason = XmCR_RESIZE;
	cbStruct.event = NULL;
	cbStruct.xform = &xform;
        cbStruct.outDevice = X_SCREEN;
	cbStruct.drawBuf = XtWindow(w);
	cbStruct.wingc = hw->h1D.gc;
	cbStruct.plotgc = hw->h1D.contentsGC;
    	XtCallCallbacks(w, XmNresizeCallback, (XtPointer)&cbStruct);
    }
}
...
{    
    ...
    cbStruct.outDevice = X_SCREEN;
    cbStruct.drawBuf = 0;
    cbStruct.wingc = w->h1D.gc;
    cbStruct.plotgc = w->h1D.contentsGC;
		    ...
		    float *uppErr, float *lowErr, int rescale)
{
    ...
    } else if (rescale == H1D_NO_RESCALE || rescale == H1D_REFRESH) {
	...
	int rescale)
{
    ...
    if (rescale == H1D_NO_RESCALE || rescale == H1D_REFRESH) {
...
static void redisplayContents(H1DWidget w, int outDevice, int redrawArea)
{
	...
	if (redrawArea == REDRAW_ALL || w->h1D.doubleBuffer) {
    	...
    	else
	{
	    char *labelfont = NULL;
	    if (w->h1D.psFont)
		XmStringGetLtoR(w->h1D.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
    	    PSDrawXmString(display, drawBuf, w->h1D.font, w->h1D.xAxisLabel,
		gc, w->h1D.xOrigin, w->h1D.axisBottom + X_LABEL_MARGIN,
		w->h1D.xEnd - w->h1D.xOrigin, XmALIGNMENT_CENTER, labelfont,
                w->h1D.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
    	...
    	else
	{
		...
		w->h1D.xEnd - w->h1D.xOrigin, XmALIGNMENT_BEGINNING, labelfont,
                w->h1D.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
...
static void drawH1D(H1DWidget w, Drawable drawBuf, int outDevice)
{
    	...
    	else
	{
	    ...
	    PSSetClipRectangle(w->h1D.xOrigin+1, w->h1D.yEnd,
			       w->h1D.xEnd, w->h1D.yOrigin-1);
...
static void computeTransform(H1DWidget w, XYTransform *xform)
{
    ...
    /* If log scaling was requested, express limits in log coordinates */
    xform->minXData = w->h1D.minXData;
    xform->minXLim = w->h1D.minXLim;
    maxXLim = w->h1D.maxXLim;
    xform->minYData = w->h1D.minYData;
    xform->minYLim = w->h1D.minYLim;
    maxYLim = w->h1D.maxYLim;
    xform->xScale = (w->h1D.xEnd - w->h1D.xOrigin) /
    	    (maxXLim - xform->minXLim);
    xform->yScale = (w->h1D.yOrigin - w->h1D.yEnd) /
    	    (maxYLim - xform->minYLim);
    xform->minXPix = w->h1D.xOrigin -
    	    (xform->minXLim - xform->minXData) * xform->xScale;
    xform->maxYPix = w->h1D.yOrigin +
    	    (xform->minYLim - xform->minYData) * xform->yScale;
    xform->xLogScaling = 0;
    xform->yLogScaling = 0;
    xform->xOrigin = w->h1D.xOrigin;
    xform->yOrigin = w->h1D.yOrigin;
    xform->xEnd = w->h1D.xEnd;
    xform->yEnd = w->h1D.yEnd;
}
