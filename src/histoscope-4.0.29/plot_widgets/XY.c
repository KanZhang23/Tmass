...
static void calcHistDataRange(XYWidget w, XYHistogram *hist, double *xMin,
	double *xMax, double *yMin, double *yMax, double *xPosMin,
	double *yPosMin);
static void computeTransform(XYWidget w, XYTransform *xform);
...
static char *DashLists[XY_N_LINE_STYLES] = {
...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(XYWidget, xy.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(XYWidget, xy.psFontSize), XmRString, "0"},
    ...
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(XYWidget, xy.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(XYWidget, xy.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(XYWidget, xy.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(XYWidget, xy.addTop), XmRString, "0"}
};
...
static float errorMarkerRatio = 1.25f;
...
static void initialize(XYWidget request, XYWidget new)
{
    ...
    if (new->xy.psFont != NULL)
    	new->xy.psFont = XmStringCopy(new->xy.psFont);
...
static void resize(XYWidget w)
{
	...
	cbStruct.wingc = w->xy.gc;
	cbStruct.plotgc = w->xy.contentsGC;
...
static void resizeParts(XYWidget w)
{
	...
	w->xy.addLeft, w->xy.addRight, w->xy.addTop, w->xy.addBottom,
	&w->xy.xMin, &w->xy.yMin, &w->xy.xMax, &w->xy.yMax);
    ...
    if (w->xy.xOrigin > w->xy.xMin + (w->xy.xMax - w->xy.xMin) * MAX_AXIS_PERCENT/100)
    	w->xy.xOrigin = w->xy.xMin + (w->xy.xMax - w->xy.xMin) * MAX_AXIS_PERCENT/100;
    ...
    if (w->xy.yAxisLabel != NULL)
    	...
    	w->xy.yEnd = w->xy.yMin + VAxisEndClearance(fs) + fs->ascent/2 + TOP_MARGIN;
...
**
** igv: fix it so that the vertical axis is not owerdrawn.
**
*/
static void resetContentsGCClipping(XYWidget w)
{
...
static void redisplay(XYWidget w, XEvent *event, Region region)
{
	...
	_XmDrawShadow (dpy, XtWindow(w), w->primitive.bottom_shadow_GC,
...
{    
    ...
    } else if (rescaleMode == XY_NO_RESCALE || rescaleMode == XY_REFRESH) {
	if (rescaleMode == XY_NO_RESCALE)
...
static void redisplayContents(XYWidget w, int outDevice, int redrawArea)
{
    ...
    /* Without double buffering, clear the drawing buffer or window only
       in the areas that have changed. The other parts are still redrawn,
       but the net effect is that the unchanged areas do not flicker. */
    if (outDevice == X_SCREEN) {
	XSetForeground(display, gc, w->core.background_pixel);
	if (redrawArea == REDRAW_ALL || w->xy.doubleBuffer) {
    	...
    	else
	{
	    char *labelfont = NULL;
	    if (w->xy.psFont)
		XmStringGetLtoR(w->xy.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
    	    PSDrawXmString(display, drawBuf, w->xy.font, w->xy.xAxisLabel,
		    gc, w->xy.xOrigin, w->xy.axisBottom + X_LABEL_MARGIN,
		    w->xy.xEnd - w->xy.xOrigin, XmALIGNMENT_CENTER, labelfont,
                    w->xy.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
	...
	fprintf(PSGetFile(), "gsave\n");
    	PSSetClipRectangle(w->xy.xOrigin+1, w->xy.yEnd,
			   w->xy.xEnd, w->xy.yOrigin-1);
    }
	...
	*curve)
{
    ...
    int errWidth = (hasMarkers?markWidth:MED_MARK_SIZE) * XYGetErrorMarkerRatio();
    	...
    	XYHistogram *histo)
{
    ...
    float *bin;
    XYErrorBar *errBar;
    int hx1, hy1, hx2, hy2, i;
    int halfErrWidth = (MED_MARK_SIZE * XYGetErrorMarkerRatio()) / 2;
    XYTransform xform;
...
const char * XYDashList(int style)
{
    if (style < 0 || style >= XY_N_LINE_STYLES)
	return NULL;
    else
	return DashLists[style];
}
...
static void drawLegend(XYWidget w, Drawable drawBuf, int outDevice)
{
    	...
    	else
	{
		...
		XmStringGetLtoR(w->xy.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
		  ...
		  nameX, top, nameWidth, XmALIGNMENT_BEGINNING, labelfont,
                  w->xy.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
	...
	double *yMax, double *xPosMin, double *yPosMin)
{
    ...
    double cMinX, cMaxX, cMinY, cMaxY, cPosMinX, cPosMinY;
    int n, hasData = False;
...
static void computeTransform(XYWidget w, XYTransform *xform)
{
    ...
    xform->xEnd = w->xy.xEnd;
    xform->yEnd = w->xy.yEnd;
}
...
/* ratio of error bar end to marker size */
void XYSetErrorMarkerRatio(float r)
{
    errorMarkerRatio = r;
}

/* ratio of error bar end to marker size */
float XYGetErrorMarkerRatio()
{
    return errorMarkerRatio;
}
