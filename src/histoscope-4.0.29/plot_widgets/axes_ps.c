...
void makePSAxis (axis *a, FILE *psfile, const char *psfont, int psfontsize)
{
    if (a == NULL) return;
    if (!a->mapped) return;
    makePSArrows (a, psfile );
    makePSTics (a->curSegment, psfile );
    makePSLabels (a->fs, a->curSegment, psfile, psfont, psfontsize);
    makePSName (a->fs, a->curSegment, psfile, psfont, psfontsize);
...
**	*psfile	Specifies output file for PostScript program being generated
**      *psfont PostScript font to use. May be NULL.
**      psfontsize   PostScript font size. May be 0.
**
...
void makePSLabels (XFontStruct * fs, labeledSegment *lS, FILE *psfile,
		   const char *psfont, int psfontsize)
{
    labelsToDraw2 *label;
    vector seg;
    vector ticDir; 
    int i, vshift;
    const char *font, *defaultFont = "Times-Roman-ISOLatin1";
    ...
    if (lS->v0 == lS->v1) return;
    if (fs == NULL && (psfont == NULL || psfontsize == 0)) return;
    ...
    if (psfont && psfontsize)
    {
	fprintf(psfile,"%d \n", psfontsize);
	vshift = 1 + psfontsize/5;
    }
    else
    {
	fprintf(psfile,"%d					\n",
...
** label->wY = (lS->tics)[label->ticNumber].y2 + (seg.x > 0 ? vshift + digHeight : -vshift);
*/	    
	    fprintf(psfile,"{						\n");
	    fprintf(psfile,"    2 index					\n");
	    fprintf(psfile,"    stringwidth				\n");
	    ...
	    if (seg.x > 0)
	    {
	        ...
	        fprintf(psfile,"    %d add			\n", vshift);
	    }
	    else
	        fprintf(psfile,"    pop %d			\n", -vshift);
	    fprintf(psfile,"    2 index					\n");
	    fprintf(psfile,"    add					\n");
