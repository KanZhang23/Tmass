...
static void redisplayContents(CellWidget w, int outDevice,
			      int redrawArea, int ignoreColor);
...
static void computeTransform(CellWidget w, XYTransform *xform);
...
static XtResource resources[] = {
    ...
    {XmNpsFont, XmCPsFont, XmRXmString, sizeof (XmString),
      XtOffset(CellWidget, cell.psFont), XmRString, NULL},
    {XmNpsFontSize, XmCPsFontSize, XmRInt, sizeof(int),
      XtOffset(CellWidget, cell.psFontSize), XmRString, "0"},
    ...
    {XmNcolorFilled, XmCcolorFilled, XmRBoolean, sizeof(Boolean),
      XtOffset(CellWidget, cell.colorFilled), XmRString, "False"},
    {XmNaddLeftMargin, XmCAddLeftMargin, XmRInt, sizeof(int),
      XtOffset(CellWidget, cell.addLeft), XmRString, "0"},
    {XmNaddRightMargin, XmCAddRightMargin, XmRInt, sizeof(int),
      XtOffset(CellWidget, cell.addRight), XmRString, "0"},
    {XmNaddBottomMargin, XmCAddBottomMargin, XmRInt, sizeof(int),
      XtOffset(CellWidget, cell.addBottom), XmRString, "0"},
    {XmNaddTopMargin, XmCAddTopMargin, XmRInt, sizeof(int),
      XtOffset(CellWidget, cell.addTop), XmRString, "0"}
};
...
static void initialize(CellWidget request, CellWidget new)
{
    ...
    if (new->cell.psFont != NULL)
	new->cell.psFont = XmStringCopy(new->cell.psFont);
...
static void destroy(CellWidget w)
{
    ...
    if (w->cell.psFont != NULL)
	XmStringFree(w->cell.psFont);
...
static void resize(CellWidget w)
{
	...
	w->cell.addLeft, w->cell.addRight, w->cell.addTop, w->cell.addBottom,
	&w->cell.xMin, &w->cell.yMin, &w->cell.xMax, &w->cell.yMax);
    ...
    if (w->cell.xOrigin > w->cell.xMin + (w->cell.xMax - w->cell.xMin) * MAX_AXIS_PERCENT/100)
    	w->cell.xOrigin = w->cell.xMin + (w->cell.xMax - w->cell.xMin) * MAX_AXIS_PERCENT/100;
	...
	cbStruct.wingc = w->cell.gc;
	cbStruct.plotgc = w->cell.contentsGC;
...
static void redisplay(CellWidget w, XEvent *event, Region region)
{
    ...
    redisplayContents(w, X_SCREEN, REDRAW_ALL, 0);
}
...
static void motionAP(CellWidget w, XEvent *event, char *args, int n_args)
{
    ...
    redisplayContents(w, X_SCREEN, redrawArea, 0);
}
/*
...
void CellSetContents(Widget w, CellRect *rects, int nRects, int rescale)
{
	...
	sw->cell.dataMinX = sw->cell.dataMaxX = 0.0;
	sw->cell.dataMinY = sw->cell.dataMaxY = 0.0;
    } else {
	...
	sw->cell.dataMinX = minX;
	sw->cell.dataMinY = minY;
	sw->cell.dataMaxX = maxX;
	sw->cell.dataMaxY = maxY;
    }
    ...
    } else if (rescale == CELL_NO_RESCALE || rescale == CELL_REFRESH) {
	if (rescale == CELL_NO_RESCALE)
    ...
    if (XtIsRealized(w))
    	redisplayContents(sw, X_SCREEN, redrawArea, rescale == CELL_REFRESH);
}
			 ...
			 double maxXLim, double maxYLim)
{
    ...
    if (XtIsRealized(w))
    	redisplayContents(sw, X_SCREEN,
    		REDRAW_V_AXIS | REDRAW_H_AXIS | REDRAW_CONTENTS, 0);
}
...
void CellZoomOut(Widget w)
{
    ...
    /* redraw if the widget is realized */
    if (XtIsRealized(w))
    	redisplayContents(sw, X_SCREEN, redrawArea, 0);
}
...
void CellPrintContents(Widget w, char *psFileName)
{
    ...
    if (ps != NULL) {
	redisplayContents(sw, PS_PRINTER, REDRAW_ALL, 0);
	EndPS();
    }    
}
...
void CellWritePS(Widget w, FILE *fp)
{
    ...
    oldFP = PSGetFile();
    PSSetFile(fp);
    redisplayContents((CellWidget)w, PS_PRINTER, REDRAW_ALL, 0);
    PSSetFile(oldFP);   
}
			      ...
			      int redrawArea, int ignoreColor)
{
    ...
    Drawable drawBuf;
    XRectangle r[4];
    double minXData = w->cell.dataMinX, minYData = w->cell.dataMinY;
    double maxXData = w->cell.dataMaxX, maxYData = w->cell.dataMaxY;
    double maxXLim = w->cell.maxXLim, minXLim = w->cell.minXLim;
    double minYLim = w->cell.minYLim, maxYLim = w->cell.maxYLim;
    double xScale, yScale;
    int nr = 0, minXPix, maxYPix, maxXPix, minYPix, low_lim, hi_lim;
	...
	if (w->cell.colorFilled && !ignoreColor && !w->cell.doubleBuffer)
	{
	    /* igv: make sure not to redraw the background over
	       the colored area. This causes terrible flicker. */
	    ...
	    minXPix = w->cell.xOrigin - (minXLim - minXData) * xScale + 1;
	    maxYPix = w->cell.yOrigin + (minYLim - minYData) * yScale;
	    maxXPix = w->cell.xOrigin - (minXLim - maxXData) * xScale;
	    minYPix = w->cell.yOrigin + (minYLim - maxYData) * yScale + 1;

	    if (redrawArea == REDRAW_ALL || (redrawArea & REDRAW_V_AXIS))
		low_lim = w->cell.axisLeft;
	    else
		low_lim = w->cell.xOrigin + 1;
	    if (minXPix > w->cell.xOrigin + 1)
		hi_lim = minXPix;
	    else
		hi_lim = w->cell.xOrigin + 1;
	    if (low_lim < hi_lim)
	    {
		r[nr].x = low_lim;
		r[nr].y = w->cell.axisTop;
		r[nr].width = hi_lim-low_lim;
		r[nr].height = w->cell.axisBottom - w->cell.axisTop;
		++nr;
	    }
	    if (redrawArea == REDRAW_ALL || (redrawArea & REDRAW_H_AXIS))
		hi_lim = w->cell.axisBottom+1;
	    else
		hi_lim = w->cell.yOrigin;
	    low_lim = maxYPix + 1;
	    if (low_lim > w->cell.yOrigin)
		low_lim = w->cell.yOrigin;
	    if (low_lim < hi_lim)
	    {
		r[nr].x = w->cell.axisLeft;
		r[nr].y = low_lim;
		r[nr].width = w->cell.axisRight-w->cell.axisLeft;
		r[nr].height = hi_lim - low_lim;
		++nr;
	    }
	    if (maxXPix < w->cell.xEnd)
	    {
		r[nr].x = maxXPix;
		r[nr].y = w->cell.axisTop;
		r[nr].width = w->cell.xEnd-maxXPix;
		r[nr].height = w->cell.axisBottom - w->cell.axisTop;
		++nr;
	    }
	    if (minYPix > w->cell.yEnd+1)
	    {
		r[nr].x = w->cell.axisLeft;
		r[nr].y = w->cell.yEnd;
		r[nr].width = w->cell.axisRight-w->cell.axisLeft;
		r[nr].height = minYPix - w->cell.yEnd;
		++nr;
	    }
	    if (nr > 0)
		XFillRectangles(display, drawBuf, gc, r, nr);
	}
	else 
	{
	    if (redrawArea == REDRAW_ALL || w->cell.doubleBuffer) {
	...
	DrawHorizontalAxis(display, drawBuf, gc, fs, outDevice,
            ...
            w->cell.psFont, w->cell.psFontSize);
    	...
    	else
	{
            char *labelfont = NULL;
	    if (w->cell.psFont)
		XmStringGetLtoR(w->cell.psFont, XmSTRING_DEFAULT_CHARSET, &labelfont);
    	    PSDrawXmString(display, drawBuf, w->cell.font, w->cell.xAxisLabel,
		gc, w->cell.xOrigin, w->cell.axisBottom + X_LABEL_MARGIN,
		w->cell.xEnd - w->cell.xOrigin, XmALIGNMENT_CENTER,
                labelfont, w->cell.psFontSize);
	    if (labelfont)
		XtFree(labelfont);
	}
    }
...
static void drawCellPlot(CellWidget w, Drawable drawBuf, int outDevice)
{
    int nPts = 0, have_color = w->cell.colorFilled;
    Display *display = XtDisplay(w);
	...
	PSSetClipRectangle(w->cell.xOrigin+1, w->cell.yEnd,
			   w->cell.xEnd, w->cell.yOrigin-1);
    }    
    for (i=0; i<w->cell.nRects; i++, rect++) {
	...
	if (have_color)
	{
   	...
   	if (x+dx > lolim && x < xMax && y+dy > yMin && y < yMax &&
    		dx > 0 && dy > 0) {
	    if (x > lolim)
		pt->x = x;
	    else
...
static void computeTransform(CellWidget w, XYTransform *xform)
{
    ...
    xform->minXData = w->cell.minXData;
    xform->minXLim = w->cell.minXLim;
    maxXLim = w->cell.maxXLim;
    xform->minYData = w->cell.minYData;
    xform->minYLim = w->cell.minYLim;
    maxYLim = w->cell.maxYLim;
    xform->xScale = (w->cell.xEnd - w->cell.xOrigin) /
    	    (maxXLim - xform->minXLim);
    xform->yScale = (w->cell.yOrigin - w->cell.yEnd) /
    	    (maxYLim - xform->minYLim);
    xform->minXPix = w->cell.xOrigin -
    	    (xform->minXLim - xform->minXData) * xform->xScale;
    xform->maxYPix = w->cell.yOrigin +
    	    (xform->minYLim - xform->minYData) * xform->yScale;
    xform->xLogScaling = 0;
    xform->yLogScaling = 0;
    xform->xOrigin = w->cell.xOrigin;
    xform->yOrigin = w->cell.yOrigin;
    xform->xEnd = w->cell.xEnd;
    xform->yEnd = w->cell.yEnd;
}
