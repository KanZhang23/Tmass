...
*									       *
* Color handling and pixmap resource leaks fixed by igv in 2003                *
*									       *
*******************************************************************************/
#include <stdio.h>
#include <assert.h>
...
typedef struct {
    ...
    Widget *lineColorBtns;
    Widget *fillColorBtns;
    Pixmap linePixmaps[XY_N_HIST_LINE_STYLES];
    Pixmap fillPixmaps[N_FILL_STYLES];
    Pixmap *lineColorPixmaps;
    Pixmap *fillColorPixmaps;
    Widget *lineOptMenus;
    Widget *fillOptMenus;
...
static void copyCurveStyles(Display *display, int screen_num,
		XYCurve *fromCurves, XYCurve *toCurves, int nCurves);
static void copyHistStyles(Display *display, int screen_num,
		XYHistogram *fromHists, XYHistogram *toHists, int nHists);
...
static void allocNewFreePrevious(Display *display, int screen_num,
			Pixel *color, int nColors, Boolean *color_used,
			int newColorIndex, Pixel prevPixel);
static int get_colors(Display *display, int screen_num, char *progname, 
		      int max_num_colors_ret, Pixel *color_pixel);
static int findFillIndex(unsigned fillStyle);
static int duplicateColor(Display *display, int screen_num,
			  Pixel in, Pixel *out);
static void histDestroyPixmaps(Display *display, histStyleDialog *dialog);
static void histFreeRest(Display *display, histStyleDialog *dialog);
static void markDestroyPixmaps(Display *display, markDialog *dialog);
static void markFreeRest(Display *display, markDialog *dialog);
	...
	XYMarkLineCallbackProc dismissCallback, void *dismissArg)
{
    ...
    Pixmap pm;
    XmString s1; 
    Pixel default_colors[MAX_COLORS];
    int n_default_colors;
    const int maxcolors = 2*nCurves + MAX_COLORS;
    ...
    dialog->lnColorPixmaps = (Pixmap *)XtMalloc(sizeof(Pixmap)*maxcolors);
    dialog->mkColorPixmaps = (Pixmap *)XtMalloc(sizeof(Pixmap)*maxcolors);
    dialog->lineColorBtns = (Widget *)XtMalloc(sizeof(Widget)*maxcolors);
    dialog->markColorBtns = (Widget *)XtMalloc(sizeof(Widget)*maxcolors);
    dialog->color = (Pixel *)XtMalloc(sizeof(Pixel)*maxcolors);
    dialog->color_used = (Boolean *)XtMalloc(sizeof(Boolean)*maxcolors);
    ...
    n_default_colors = get_colors(display, screen_num, "XYDialogs",
				  MAX_COLORS, default_colors);
    if (n_default_colors > 0)
    {
	for (i=0; i<nCurves; ++i)
	{
	    if (findinList(default_colors, n_default_colors, curves[i].linePixel) < 0 && 
		findinList(dialog->color, dialog->nColors, curves[i].linePixel) < 0)
	    {
		if (duplicateColor(display, screen_num, curves[i].linePixel,
				   dialog->color + dialog->nColors))
		{
		    markFreeRest(display, dialog);
		    return NULL;
		}
		++dialog->nColors;
	    }
	    if (findinList(default_colors, n_default_colors, curves[i].markerPixel) < 0 &&
		findinList(dialog->color, dialog->nColors, curves[i].markerPixel) < 0)
	    {
		if (duplicateColor(display, screen_num, curves[i].markerPixel,
				   dialog->color + dialog->nColors))
		{
		    markFreeRest(display, dialog);
		    return NULL;
	...
	for (i=0; i<n_default_colors; ++i)
	    dialog->color[dialog->nColors++] = default_colors[i];
    }
	...
	XtVaSetValues(XmOptionButtonGadget(lineOptMenu), XmNmarginHeight, 0, NULL);
	...
	if (dialog->nColors > 0) {
	    ...
	    j = findinList(dialog->color, dialog->nColors, curves[i].linePixel);
	    if (j >= 0) {
	...
	XtVaSetValues(XmOptionButtonGadget(markOptMenu), XmNmarginHeight, 0, NULL);
	...
	XtVaSetValues(XmOptionButtonGadget(sizeOptMenu), XmNmarginHeight, 0, NULL);
	...
	if (dialog->nColors > 0) {
	    ...
	    j = findinList(dialog->color, dialog->nColors, curves[i].markerPixel);
	    if (j >= 0) {
    ...
    XtVaGetValues(parent, XmNbackground, &parentBGColor, NULL);
    ...
    name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
		...
		XmNbottomWidget, bottomWidget,
    		...
    		XmNrightWidget, lineOptMenu, NULL);
    XmStringFree(s1);
    if (dialog->nColors > 0) {
	name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
		    ...
		    XmNbottomWidget, bottomWidget,
    		    ...
    		    XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		    XmNrightWidget, lnColorOptMenu, NULL);
    	XmStringFree(s1);
    }
    name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
    		XmNlabelString, s1=XmStringCreateSimple("Style"),
		...
		XmNbottomWidget, bottomWidget,
    		...
    		XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		XmNrightWidget, markOptMenu, NULL);
    XmStringFree(s1);
    name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
		...
		XmNbottomWidget, bottomWidget,
    		...
    		XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		XmNrightWidget, sizeOptMenu, NULL);
    XmStringFree(s1);
    if (dialog->nColors > 0) {
	name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
    		    XmNlabelString, s1=XmStringCreateSimple("Color"),
		    ...
		    XmNbottomWidget, bottomWidget,
    		    ...
    		    XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		    XmNrightWidget, mkColorOptMenu, NULL);
    	XmStringFree(s1);
    }
    ...
    XtVaSetValues(clearMarks, XmNdefaultButtonShadowThickness, 0, NULL);
    XtVaSetValues(clearLines, XmNdefaultButtonShadowThickness, 0, NULL);
    XtVaSetValues(defaultMarks, XmNdefaultButtonShadowThickness, 0, NULL);
    XtVaSetValues(defaultLines, XmNdefaultButtonShadowThickness, 0, NULL);
    XtVaSetValues(markSize, XmNdefaultButtonShadowThickness, 0, NULL);
    if (dialog->nColors > 0)
	XtVaSetValues(copyColors, XmNdefaultButtonShadowThickness, 0, NULL);
    	...
    	XYHistCallbackProc dismissCallback, void *dismissArg)
{
    ...
    Widget fillColorOptMenu, lineColorOptMenu, bottomWidget;
    XtArgVal bottomAttach;
    int i, n, j;
    Arg args[12];
    ...
    Pixel default_colors[MAX_COLORS];
    int n_default_colors;
    const int maxcolors = 2*nHistograms+MAX_COLORS;
    ...
    dialog->dismissArg = dismissArg;
    dialog->lineColorBtns = (Widget *)XtMalloc(sizeof(Widget)*maxcolors);
    dialog->fillColorBtns = (Widget *)XtMalloc(sizeof(Widget)*maxcolors);
    dialog->lineColorPixmaps = (Pixmap *)XtMalloc(sizeof(Pixmap)*maxcolors);
    dialog->fillColorPixmaps = (Pixmap *)XtMalloc(sizeof(Pixmap)*maxcolors);
    dialog->color = (Pixel *)XtMalloc(sizeof(Pixel)*maxcolors);
    dialog->color_used = (Boolean *)XtMalloc(sizeof(Boolean)*maxcolors);
    for (i=0; i<maxcolors; i++)
    	dialog->color_used[i] = FALSE;
	...
	{
	    if (findinList(default_colors, n_default_colors, histograms[i].linePixel) < 0 && 
		findinList(dialog->color, dialog->nColors, histograms[i].linePixel) < 0)
	    {
		if (duplicateColor(display, screen_num, histograms[i].linePixel,
				   dialog->color + dialog->nColors))
		{
		    histFreeRest(display, dialog);
		    return NULL;
		}
		++dialog->nColors;
	    }
	    if (findinList(default_colors, n_default_colors, histograms[i].fillPixel) < 0 &&
		findinList(dialog->color, dialog->nColors, histograms[i].fillPixel) < 0)
	    {
		if (duplicateColor(display, screen_num, histograms[i].fillPixel,
				   dialog->color + dialog->nColors))
		{
		    histFreeRest(display, dialog);
		    return NULL;
	...
	if (dialog->nColors > 0) {
	    ...
	    j = findinList(dialog->color, dialog->nColors, histograms[i].linePixel);
	    if (j >= 0) {
 	        /* printf("Line Button %d: Found pixel value %d as index %d\n",
	...
	XtVaSetValues(XmOptionButtonGadget(fillOptMenu), XmNmarginHeight, 0, NULL);
	...
	name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
		    ...
		    XmNbottomWidget, bottomWidget,
    		    ...
    		    XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		    XmNrightWidget, lineColorOptMenu, NULL);
    	XmStringFree(s1);
    }
    name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
    		XmNlabelString, s1=XmStringCreateSimple("Style"),
		...
		XmNbottomWidget, bottomWidget,
    		...
    		XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		XmNrightWidget, fillOptMenu, NULL);
    XmStringFree(s1);
    if (dialog->nColors > 0) {
	name = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
    		    XmNlabelString, s1=XmStringCreateSimple("Color"),
		    ...
		    XmNbottomWidget, bottomWidget,
    		    ...
    		    XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, 
    		    XmNrightWidget, fillColorOptMenu, NULL);
    	XmStringFree(s1);
    }
    name1 = XtVaCreateManagedWidget("name", xmLabelGadgetClass, form,
    	...
    	Widget btns[XY_N_HIST_LINE_STYLES], XtCallbackProc cbProc, void *cbArg)
{
    ...
    XtVaGetValues(parent, XmNbackground, &bgColor, NULL);
    	...
    	XtCallbackProc cbProc, void *cbArg)
{
    ...
    for (i=0; i<nColors; i++) {
        ...
        XSetForeground(display, drawGC, bgColor);
        XFillRectangle(display, pm, drawGC, 0, 0, 50, 12);
        XYDrawLine(display, pm, drawGC, XY_X_THICK_LINE, colors[i], 5,
            	6, 45, 6);
...
static void updateMarkLineSample(markDialog *dialog, int index)
{
    ...
    XtVaSetValues(dialog->sampleLabels[index], XmNlabelPixmap, 
    	    dialog->linePixmaps[0], NULL);
    XtVaSetValues(dialog->sampleLabels[index], XmNlabelPixmap, pm, NULL);
}
...
static void updateMarkLineDialog(markDialog *dialog)
{
    ...
    for (i=0, curve=dialog->curves; i<dialog->nCurves; i++, curve++) {
        XtVaGetValues(dialog->lineOptMenus[i], XmNmenuHistory, &selectedW, NULL);
        ...
        XtVaGetValues(dialog->markOptMenus[i], XmNmenuHistory, &selectedW, NULL);
        ...
        XtVaGetValues(dialog->sizeOptMenus[i], XmNmenuHistory, &selectedW, NULL);
...
static void updateHistDialog(histStyleDialog *dialog)
{
    ...
    for (i=0, hist=dialog->hists; i<dialog->nHists; i++, hist++) {
        ...
        XtVaGetValues(dialog->fillOptMenus[i], XmNmenuHistory, &selectedW, NULL);
        ...
        if (dialog->nColors > 0) {
            XtVaGetValues(dialog->lineColorOptMenus[i], XmNmenuHistory, 
            	&selectedW, NULL);
            for (j=0, w=dialog->lineColorBtns; j<dialog->nColors; j++, w++) {
            ...
            XtVaGetValues(dialog->fillColorOptMenus[i], XmNmenuHistory, 
            	&selectedW, NULL);
            for (j=0, w=dialog->fillColorBtns; j<dialog->nColors; j++, w++) {
...
static void defaultLinesCB(Widget w, markDialog *dialog, caddr_t callData)
{
    ...
    for (i=0; i<dialog->nCurves; i++)
	XtVaSetValues(dialog->lineOptMenus[i], XmNmenuHistory,
		dialog->lineBtns[(i % XY_4_DOT_DASH) + 1], NULL);
    updateMarkLineDialog(dialog);
}
...
static void defaultMarksCB(Widget w, markDialog *dialog, caddr_t callData)
{
    ...
    for (i=0; i<dialog->nCurves; i++)
	XtVaSetValues(dialog->markOptMenus[i], XmNmenuHistory,
		dialog->markBtns[(i % XY_SOLID_CIRCLE_MARK) + 1], NULL);
    updateMarkLineDialog(dialog);
}
...
static void copyColorsCB(Widget w, markDialog *dialog, caddr_t callData)
{
    ...
    for (i=0; i<dialog->nCurves; i++) {
	XtVaGetValues(dialog->lnColorOptMenus[i], XmNmenuHistory, &selectedW, NULL);
        for (j=0, wid=dialog->lineColorBtns; j<dialog->nColors;
        	    j++, wid++) {
            if (*wid == selectedW) {
        	XtVaSetValues(dialog->mkColorOptMenus[i], XmNmenuHistory,
		    dialog->markColorBtns[j], NULL);
        	break;
    	    }
    	}
    }
...
static void markLineOkCB(Widget w, markDialog *dialog, caddr_t callData)
{
    ...
    copyCurveStyles(display, screen_num, dialog->curves,
		    dialog->origCurves, dialog->nCurves);
    if (dialog->okCB != NULL)
...
static void histOkCB(Widget w, histStyleDialog *dialog, caddr_t callData)
{
    ...
    copyHistStyles(display, screen_num, dialog->hists,
		   dialog->origHists, dialog->nHists);
    if (dialog->okCB != NULL)
...
static void markDestroyPixmaps(Display *display, markDialog *dialog)
{
    ...
    for (i=0; i<dialog->nColors; ++i)
    {
	XFreePixmap(display, dialog->lnColorPixmaps[i]);
	XFreePixmap(display, dialog->mkColorPixmaps[i]);
    }
    for (i=0; i<dialog->nCurves; ++i)
	XFreePixmap(display, dialog->samplePixmaps[i]);
}
...
static void markLineDestroyCB(Widget w, markDialog *dialog, caddr_t callData)
{
    ...
    markDestroyPixmaps(display, dialog);
    markFreeRest(display, dialog);
}
...
static void markFreeRest(Display *display, markDialog *dialog)
{
	...
	XFreeColors(display, DefaultColormap(display, screen_num),
		    dialog->color, dialog->nColors, planes);
    }
    ...
    XtFree((char *)dialog->lnColorPixmaps);
    XtFree((char *)dialog->mkColorPixmaps);
    XtFree((char *)dialog->lineColorBtns);
    XtFree((char *)dialog->markColorBtns);
    XtFree((char *)dialog->color);
    XtFree((char *)dialog->color_used);
...
static void histDestroyPixmaps(Display *display, histStyleDialog *dialog)
{
    ...
    for (i=0; i<dialog->nColors; ++i)
    {
	XFreePixmap(display, dialog->lineColorPixmaps[i]);
	XFreePixmap(display, dialog->fillColorPixmaps[i]);
    }
    for (i=0; i<dialog->nHists; ++i)
	XFreePixmap(display, dialog->samplePixmaps[i]);
}
...
static void histFreeRest(Display *display, histStyleDialog *dialog)
{
    ...
    XFreeGC(display, dialog->gc);
    XtFree((char *)dialog->lineColorBtns);
    XtFree((char *)dialog->fillColorBtns);
    XtFree((char *)dialog->lineColorPixmaps);
    XtFree((char *)dialog->fillColorPixmaps);
    XtFree((char *)dialog->color);
    XtFree((char *)dialog->color_used);
...
static void histDestroyCB(Widget w, histStyleDialog *dialog, caddr_t callData)
{
    ...
    histDestroyPixmaps(display, dialog);
    histFreeRest(display, dialog);
}

static void copyCurveStyles(Display *display, int screen_num,
			    XYCurve *fromCurves, XYCurve *toCurves,
			    int nCurves)
{
    int i;
    XYCurve *from, *to;
	...
	if (duplicateColor(display, screen_num, from->markerPixel, &pix) == 0)
	{
	    XFreeColors(display, cmap, &to->markerPixel, 1, planes);
	    to->markerPixel = pix;
	}
	if (duplicateColor(display, screen_num, from->linePixel, &pix) == 0)
	{
	    XFreeColors(display, cmap, &to->linePixel, 1, planes);
	    to->linePixel = pix;
	}
    }
}
...
static void copyHistStyles(Display *display, int screen_num,
			   XYHistogram *fromHists, XYHistogram *toHists,
			   int nHists)
{
    int i;
    XYHistogram *from, *to;
	...
	to->lineStyle = from->lineStyle;
	if (duplicateColor(display, screen_num, from->fillPixel, &pix) == 0)
	{
	    XFreeColors(display, cmap, &to->fillPixel, 1, planes);
	    to->fillPixel = pix;
	}
	if (duplicateColor(display, screen_num, from->linePixel, &pix) == 0)
	{
...
static void createMarkSizeDialog(markDialog *dialog)
{
    ...
    XtVaSetValues(XtParent(selBox), XmNtitle, "Marker Size", NULL);
    ...
    for (i=0; i<XY_N_MARK_SIZES; i++) {
    	dialog->sizeDlogBtns[i] = XtVaCreateManagedWidget("size",
    		...
    		XmNlabelPixmap, dialog->sizePixmaps[i], XmNset, i==avgSize, NULL);
    }
#if XmVersion >= 1002
    XtVaSetValues(checkBox, XmNinitialFocus, dialog->sizeDlogBtns[avgSize], NULL);
#endif
 ...
 *
 * igv: this operation makes no sense when there are more
 *       than MAX_COLORS colors. All code removed.
 */
static void allocNewFreePrevious(Display *display, int screen_num,
				 Pixel *color, int nColors, Boolean *color_used,
				 int newColorIndex, Pixel prevPixel)
...
*               handle color; otherwise returns 0.  Returns MAX_COLORS or at
*               most max_num_colors_ret pixel values of the colors allocated
*               in array color_pixel.  Also returns the exact definitions.
*
...
/* 	printf("The RGB values actually allocated are %d, %d, %d\n", 
	    ...
	    exact_def.blue);
 */
	color_pixel[ncolors] = exact_def.pixel;
        ncolors++;
    }
...
{
    return XYCreateCurveStylesDialog(parent, curves, nCurves, okCallback, okArg,
    	    applyCallback, applyArg, dismissCallback, dismissArg);
}
int XYEditStyles(Widget parent, XYCurve *curves, int nCurves)
{
    return XYEditCurveStyles(parent, curves, nCurves);
}
