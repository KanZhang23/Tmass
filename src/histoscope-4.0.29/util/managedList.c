    	...
    	void *setDialogDataArg, void (*freeItemCB)(void *))
{
    ...
    placeholderTable = StringTable(placeholderStr);
...
static int incorporateDialogData(managedListData *ml, int listPos, int explicit)
{
    ...
    if (listPos == 1) {
#ifdef NEW_ITEMS_FIRST
	for (i= *ml->nItems; i>=1; i--)
    	    ml->itemList[i] = ml->itemList[i-1];
	ml->itemList[listPos-1] = item;
	(*ml->nItems)++;
	selectedPos = selectedListPosition(ml);
	updateDialogFromList(ml, selectedPos==1 ? -1 : selectedPos-1);
#else
