...
#include "npstat/stat/convertAxis.hh"
...
namespace npstat {
    ...
    NUHistoAxis convertToHistoAxis(const GridAxis& gridAxis, double xMin)
    {
        ...
        for (unsigned i=0; i<nCoords; ++i)
        {
            ...
            if (x <= xMin)
                throw std::invalid_argument("In npstat::convertToHistoAxis: "
    ...
    {
        if (histoAxis.isUniform())
        {
            const unsigned nBins = histoAxis.nBins();
            const double xmin = histoAxis.binCenter(0);
            const double xmax = histoAxis.binCenter(nBins - 1);
            return DualAxis(nBins, xmin, xmax, histoAxis.label().c_str());
        }
        else
        {
            const unsigned nBins = histoAxis.nBins();
            ...
            for (unsigned i=0; i<nBins; ++i)
                coords.push_back(histoAxis.binCenter(i));
            return DualAxis(coords, histoAxis.label().c_str());
        }
    }
}
