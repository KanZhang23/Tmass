...
namespace npstat {
    inline void SobolGenerator::update()
    {
        if (count++)
        {
            unsigned l = 0U;
            for (long long n = count - 2; n & 1LL; n>>=1, ++l);
            if (l >= maxcol)
                throw std::runtime_error(
                    "In npstat::SobolGenerator::update: "
                    "too many random numbers requested");
            for (unsigned i=0; i<dim_num; ++i)
                lastq[i] ^= v[i][l];
        }
    }

    SobolGenerator::SobolGenerator(const unsigned dim,
                                   const unsigned maxPowerOfTwo,
                                   const unsigned nSkip)
        : count(0), 
          dim_num(dim),
          maxcol(maxPowerOfTwo)
    {
        if (!(dim_num >= 1U && dim_num <= DIM_MAX)) throw std::runtime_error(
            "In npstat::SobolGenerator constructor: "
            "unsupported random vector dimensionality");
        if (!(maxcol >= 2 && maxcol <= LOG_MAX)) throw std::runtime_error(
            "In npstat::SobolGenerator constructor: "
            "expected number of random vectors is out of range");

        static const int poly[DIM_MAX] = {
        ...
        for (unsigned i=0; i<DIM_MAX; ++i)
        {
            v[i][0] = 1;
            for (unsigned j=1; j<LOG_MAX; ++j)
                v[i][j] = 0;
        }
        ...
        for (unsigned i = 1; i < dim_num; ++i)
        {
            ...
            bool includ[LOG_MAX] = {false,};
            j = poly[i];
            for (long long k = m-1; 0 <= k; --k)
            {
                long long j2 = j / 2;
                includ[k] = ( j != ( 2 * j2 ) );
                j = j2;
        ...
        long long l = 1;
        for (int j = static_cast<int>(maxcol)-2; j >= 0; --j)
        {
            l *= 2;
            for (unsigned i = 0; i < dim_num; ++i)
                v[i][j] *= l;
    ...
    void SobolGenerator::run(double* buf, const unsigned bufDim,
                             const unsigned nPoints)
    {
        if (nPoints)
        {
            if (bufDim < nPoints*dim_num) throw std::invalid_argument(
                "In npstat::SobolGenerator::run: "
    ...
    void SobolGenerator::nextBitSet(long long* buf, unsigned bufSize)
    {
        // Yield the set of bits for one point
        if (bufSize < dim_num) throw std::invalid_argument(
            "In npstat::SobolGenerator::nextBitSet: "
            "insufficient length of the output buffer");
        assert(buf);
        update();
        for (unsigned i=0; i<dim_num; ++i)
