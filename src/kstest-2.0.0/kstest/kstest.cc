...
static const double LnFactorial[MFACT + 1] = {
   0.,
   0.,
   0.6931471805599453,
   1.791759469228055,
...
static double getLogFactorial (const unsigned n)
{
   /* Returns the natural logarithm of factorial n! */
   if (n <= MFACT) {
      return LnFactorial[n];
   } else {
      const double x = (double) (n + 1);
...
static double rapfac (const unsigned n)
{
   /* Computes n! / n^n */
   long double res = 1.0L / n;
...
static double KSPlusbarAsymp (const unsigned n, const double x)
{
   /* Compute the probability of the KS+ distribution using an asymptotic
      formula */
   const double t = (6.0 * n * x + 1);
   const double z = t * t / (18.0 * n);
...
static double KSPlusbarUpper (const unsigned n, const double x)
{
...
static double Pelz (const unsigned n, const double x)
{
...
static double Pomeranz (const int n, const double x)
{
   ...
   std::vector<double> membuf(3 * (2U * n + 3), 0.0);
   A = &membuf[0];
   Atflo = A + (2 * n + 3);
   Atcei = Atflo + (2 * n + 3);

   // Memory for matrix-like calculations
   std::vector<double> matbuf(6*(n + 2U));
   std::vector<double*> ptrbuf(6);
   for (i=0; i<6; ++i)
       ptrbuf[i] = &matbuf[0] + i*(n + 2U);
   V = &ptrbuf[0];
   H = V + 2;
...
static double cdfSpecial (const unsigned n, const double x)
{
   ...
   if (x <= 1.0 / n) {
      const double t = 2.0 * x * n - 1.0;
      if (n <= NEXACT) {
         const double w = rapfac (n);
         return w * pow (t, (double) n);
      } else {
         const double w = getLogFactorial (n) + n * log (t / n);
         return exp (w);
      }
   }
...
static double fbarSpecial (const unsigned n, const double x)
{
   if (!n)
      return 1.0;
   if (x >= 1.0)
   ...
   if (x <= 1.0 / n) {
      const double t = 2.0 * x * n - 1.0;
      if (n <= NEXACT) {
         const double z = rapfac (n);
         return 1.0 - z * pow (t, (double) n);
      } else {
         const double z = getLogFactorial (n) + n * log (t / n);
         return 1.0 - exp (z);
      }
   }
...
namespace kstest {
    ...
    double KScdf (const unsigned n, const double x)
    {
        const double u = cdfSpecial (n, x);
        if (u >= 0.0)
            return u;
...
/*========================================================================*/
    ...
    double KSfbar (const unsigned n, const double x)
    {
        const double v = fbarSpecial (n, x);
        if (v >= 0.0)
            return v;
...
Explicit memory allocations are replaced by use of std::vector
by Igor Volobouev.
...
/* Matrix product */
static void mMultiply (const double *A, const double *B, double *C, int m);

/* Matrix power */
static void mPower (const double *A, int eA, double *V, int *eV, int m, int n);
...
static double DurbinMatrix (const int n, const double d)
{
   ...
   m = 2 * k - 1;
   h = k - n * d;
   std::vector<double> membuf(2 * m * m);
   H = &membuf[0];
   Q = H + m * m;
   for (i = 0; i < m; i++)
      for (j = 0; j < m; j++)
...
static void mMultiply (const double *A, const double *B, double *C, const int m)
{
   int i, j, k;
   double s;
   for (i = 0; i < m; i++)
...
static void renormalize (double *V, const int m, int *p)
{
   const int msq = m * m;
   for (int i = 0; i < msq; i++)
      V[i] *= INORM;
...
static void mPower (const double *A, const int eA,
                    double *V, int *eV,
