...
proc ::proctrace {args} {
	...
	if {[info exists proctrace_guard_UnlIkELY_VARiAbLE_NaME]} {
        ...
        # Trigger clock procedures now so that we do not
        # get into trouble with calling "unknown" handlers
        # when we are changing procedure bodies
	...
	proc ::proctrace_existing_procs {namsp} {
            # Avoid a few namespaces for now which cause
            # tracing problems in tcl 8.5.
            set problem_namespaces [list "::tcl::dict" "::tcl::clock"]
            if {[lsearch -exact $problem_namespaces $namsp] >= 0} {
                return
            }
