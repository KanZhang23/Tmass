      DOUBLE PRECISION FUNCTION P0_DSIG97(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ d~ > e+ ve d u~ d~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e+ ve s c~ s~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX97(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG97=PD(0)*CONV*DSIGUU
      ELSE
