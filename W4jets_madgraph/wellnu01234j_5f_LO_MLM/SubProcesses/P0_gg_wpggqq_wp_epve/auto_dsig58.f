      DOUBLE PRECISION FUNCTION P0_DSIG58(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u d~ > e+ ve u c u~ c~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u d~ > e+ ve u s u~ s~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u d~ > e+ ve u b u~ b~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c s~ > e+ ve c d c~ d~
      PD(IPROC)=C1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c s~ > e+ ve c b c~ b~
      PD(IPROC)=C1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX58(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG58=PD(0)*CONV*DSIGUU
      ELSE
