      DOUBLE PRECISION FUNCTION P0_DSIG66(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u s~ > e+ ve g g u c~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > e+ ve g g c u~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d s~ > e+ ve g g d c~
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s d~ > e+ ve g g s u~
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX66(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG66=PD(0)*CONV*DSIGUU
      ELSE
