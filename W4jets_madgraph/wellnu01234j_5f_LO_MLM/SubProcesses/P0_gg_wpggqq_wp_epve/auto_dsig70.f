      DOUBLE PRECISION FUNCTION P0_DSIG70(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u s~ > e+ ve u s c~ s~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > e+ ve c d u~ d~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d s~ > e+ ve d s c~ s~
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s d~ > e+ ve s d u~ d~
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX70(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG70=PD(0)*CONV*DSIGUU
      ELSE
