      DOUBLE PRECISION FUNCTION P0_DSIG88(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ d~ > e+ ve s u~ c~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > e+ ve d c~ u~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX88(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG88=PD(0)*CONV*DSIGUU
      ELSE
