      DOUBLE PRECISION FUNCTION P0_DSIG49(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u c~ > e+ ve u s c~ c~
      PD(IPROC)=U1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c u~ > e+ ve c d u~ u~
      PD(IPROC)=C1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > e+ ve d s c~ c~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > e+ ve s d u~ u~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX49(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG49=PD(0)*CONV*DSIGUU
      ELSE
