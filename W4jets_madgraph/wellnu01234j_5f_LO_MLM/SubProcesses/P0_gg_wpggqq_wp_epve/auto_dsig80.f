      DOUBLE PRECISION FUNCTION P0_DSIG80(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u~ > e+ ve d u~ u~ u~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > e+ ve s c~ c~ c~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX80(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG80=PD(0)*CONV*DSIGUU
      ELSE
