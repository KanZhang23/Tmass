      DOUBLE PRECISION FUNCTION P0_DSIG51(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u c~ > e+ ve d d c~ d~
      PD(IPROC)=U1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u s~ > e+ ve d d s~ d~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c u~ > e+ ve s s u~ s~
      PD(IPROC)=C1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > e+ ve s s d~ s~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX51(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG51=PD(0)*CONV*DSIGUU
      ELSE
