      DOUBLE PRECISION FUNCTION P0_DSIG60(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u d~ > e+ ve u s c~ d~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c s~ > e+ ve c d u~ s~
      PD(IPROC)=C1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d u~ > e+ ve d s c~ u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > e+ ve s d u~ c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX60(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG60=PD(0)*CONV*DSIGUU
      ELSE
