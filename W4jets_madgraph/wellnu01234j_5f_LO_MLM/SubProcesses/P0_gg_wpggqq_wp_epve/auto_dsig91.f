      DOUBLE PRECISION FUNCTION P0_DSIG91(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ s~ > e+ ve c u~ c~ c~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > e+ ve u u~ c~ u~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ s~ > e+ ve c c~ d~ c~
      PD(IPROC)=DX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX91(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG91=PD(0)*CONV*DSIGUU
      ELSE
