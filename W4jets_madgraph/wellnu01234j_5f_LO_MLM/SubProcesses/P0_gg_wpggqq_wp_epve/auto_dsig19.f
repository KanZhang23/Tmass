      DOUBLE PRECISION FUNCTION P0_DSIG19(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u > e+ ve u u s c~
      PD(IPROC)=U1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c > e+ ve c c d u~
      PD(IPROC)=C1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d > e+ ve d d s c~
      PD(IPROC)=D1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s > e+ ve s d s u~
      PD(IPROC)=S1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX19(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG19=PD(0)*CONV*DSIGUU
      ELSE
