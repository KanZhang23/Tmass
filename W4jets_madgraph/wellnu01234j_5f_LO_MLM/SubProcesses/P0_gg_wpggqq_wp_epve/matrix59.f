...
C     ----------
      ...
      DO IHEL=1,NCOMB
        IF (GOODHEL(IHEL) .OR. NTRY .LT. 2) THEN
          T=P0_MATRIX59(P ,NHEL(1,IHEL),JC(1))
          ANS=ANS+T
      ...
      REAL*8 FUNCTION P0_MATRIX59(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.2.2, 2014-11-06
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 44
      ...
      CALL FFV1_2(W(1,1),W(1,9),GC_11,ZERO,ZERO,W(1,3))
...
C     Amplitude(s) for diagram number 47
      ...
      CALL FFV1P0_3(W(1,1),W(1,25),GC_11,ZERO,ZERO,W(1,19))
...
C     Amplitude(s) for diagram number 60
      CALL FFV1_0(W(1,1),W(1,18),W(1,27),GC_11,AMP(60))
...
C     Amplitude(s) for diagram number 64
      CALL FFV1_0(W(1,1),W(1,8),W(1,29),GC_11,AMP(64))
      JAMP(1)=+1D0/4D0*(-1D0/9D0*AMP(1)-1D0/9D0*AMP(2)-1D0/3D0*AMP(3)
     ...
     $ -AMP(63))
      JAMP(2)=+1D0/4D0*(+1D0/3D0*AMP(1)+1D0/3D0*AMP(2)+AMP(4)
     ...
     $ *AMP(63)+1D0/3D0*AMP(64))
      JAMP(3)=+1D0/4D0*(+AMP(3)+AMP(5)+IMAG1*AMP(7)+1D0/3D0*AMP(8)
     ...
     $ *AMP(60)+AMP(61))
      JAMP(4)=+1D0/4D0*(-AMP(2)-1D0/3D0*AMP(3)-1D0/3D0*AMP(4)
     ...
     $ -1D0/3D0*AMP(62)-AMP(64))
      JAMP(5)=+1D0/4D0*(-AMP(1)-1D0/3D0*AMP(5)-1D0/3D0*AMP(6)-AMP(8)
     ...
     $ -1D0/9D0*AMP(64))
      JAMP(6)=+1D0/4D0*(+1D0/3D0*AMP(1)+1D0/3D0*AMP(2)+1D0/9D0*AMP(3)
     ...
     $ *AMP(64))
      ...
      DO I = 1, NCOLOR
        ...
        ENDDO
        P0_MATRIX59 = P0_MATRIX59+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
      END
