      DOUBLE PRECISION FUNCTION P0_DSIG39(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > e+ ve g g s c~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > e+ ve g g d u~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > e+ ve g g s c~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > e+ ve g g d u~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX39(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG39=PD(0)*CONV*DSIGUU
      ELSE
