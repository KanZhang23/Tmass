      DOUBLE PRECISION FUNCTION P0_DSIG74(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d u~ > e+ ve d d u~ u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > e+ ve s s c~ c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX74(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG74=PD(0)*CONV*DSIGUU
      ELSE
