      DOUBLE PRECISION FUNCTION P0_DSIG82(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ c~ > e+ ve d u~ u~ c~
      PD(IPROC)=UX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ s~ > e+ ve d u~ u~ s~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > e+ ve s c~ c~ d~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX82(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG82=PD(0)*CONV*DSIGUU
      ELSE
