      DOUBLE PRECISION FUNCTION P0_DSIG50(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u c~ > e+ ve c d c~ c~
      PD(IPROC)=U1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u s~ > e+ ve s d s~ s~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c u~ > e+ ve u s u~ u~
      PD(IPROC)=C1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > e+ ve d s d~ d~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX50(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG50=PD(0)*CONV*DSIGUU
      ELSE
