      DOUBLE PRECISION FUNCTION P0_DSIG87(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ d~ > e+ ve d u~ u~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > e+ ve s c~ c~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX87(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG87=PD(0)*CONV*DSIGUU
      ELSE
