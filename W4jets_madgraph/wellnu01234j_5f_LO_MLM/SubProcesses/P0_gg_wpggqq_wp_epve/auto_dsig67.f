      DOUBLE PRECISION FUNCTION P0_DSIG67(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u s~ > e+ ve u u u~ c~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > e+ ve c c c~ u~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d s~ > e+ ve d d d~ c~
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s d~ > e+ ve s s s~ u~
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX67(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG67=PD(0)*CONV*DSIGUU
      ELSE
