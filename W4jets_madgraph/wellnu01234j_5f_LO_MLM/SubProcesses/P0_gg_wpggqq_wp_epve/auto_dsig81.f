      DOUBLE PRECISION FUNCTION P0_DSIG81(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u~ > e+ ve s u~ u~ c~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > e+ ve d c~ u~ c~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ d~ > e+ ve s d~ c~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e+ ve d s~ u~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX81(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG81=PD(0)*CONV*DSIGUU
      ELSE
