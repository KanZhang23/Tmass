      DOUBLE PRECISION FUNCTION P0_DSIG12(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u~ > e+ ve g s u~ c~
      PD(IPROC)=G1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c~ > e+ ve g d c~ u~
      PD(IPROC)=G1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > e+ ve g s d~ c~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > e+ ve g d s~ u~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP0_MATRIX12(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        P0_DSIG12=PD(0)*CONV*DSIGUU
      ELSE
