      DOUBLE PRECISION FUNCTION P1_DSIG84(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u~ > e- ve~ c u~ u~ s~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > e- ve~ u c~ c~ d~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ d~ > e- ve~ c d~ d~ s~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e- ve~ u s~ d~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP1_MATRIX84(PP,P1_DSIGUU)
      IF (P1_DSIGUU.LT.1D199) THEN
        P1_DSIG84=PD(0)*CONV*P1_DSIGUU
      ELSE
