      DOUBLE PRECISION FUNCTION P1_DSIG50(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d > e- ve~ u u d u~
      PD(IPROC)=D1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s > e- ve~ c c s c~
      PD(IPROC)=S1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP1_MATRIX50(PP,P1_DSIGUU)
      IF (P1_DSIGUU.LT.1D199) THEN
        P1_DSIG50=PD(0)*CONV*P1_DSIGUU
      ELSE
