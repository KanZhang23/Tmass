      DOUBLE PRECISION FUNCTION P1_DSIG98(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ d~ > e- ve~ u u~ d~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > e- ve~ c c~ s~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP1_MATRIX98(PP,P1_DSIGUU)
      IF (P1_DSIGUU.LT.1D199) THEN
        P1_DSIG98=PD(0)*CONV*P1_DSIGUU
      ELSE
