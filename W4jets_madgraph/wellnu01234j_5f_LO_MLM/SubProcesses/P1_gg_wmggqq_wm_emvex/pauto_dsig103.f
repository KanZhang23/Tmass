      DOUBLE PRECISION FUNCTION P1_DSIG103(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ d~ > e- ve~ u d~ d~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e- ve~ c s~ s~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP1_MATRIX103(PP,P1_DSIGUU)
      IF (P1_DSIGUU.LT.1D199) THEN
        P1_DSIG103=PD(0)*CONV*P1_DSIGUU
      ELSE
