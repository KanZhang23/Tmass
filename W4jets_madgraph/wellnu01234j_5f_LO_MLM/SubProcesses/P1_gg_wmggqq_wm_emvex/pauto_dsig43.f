      DOUBLE PRECISION FUNCTION P1_DSIG43(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u c~ > e- ve~ u c c~ s~
      PD(IPROC)=U1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c u~ > e- ve~ c u u~ d~
      PD(IPROC)=C1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > e- ve~ d c c~ s~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > e- ve~ s u u~ d~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      CALL SP1_MATRIX43(PP,P1_DSIGUU)
      IF (P1_DSIGUU.LT.1D199) THEN
        P1_DSIG43=PD(0)*CONV*P1_DSIGUU
      ELSE
